...
export class InventoryEngine {
  ...
  async searchVehicles(params: {
    ...
    const { tenantId, locationId, query, year, maxPrice, maxMonthlyPayment } =
      params;
  ...
  // VIN COMPARISON
  // ─────────────────────────────
  async getVehiclesByVin(tenantId: string, vins: string[]) {
  ...
  // ─────────────────────────────
  async getVehicleByVin(tenantId: string, vin: string) {
    ...
    return this.photoService.enrich({
      ...
      status: vehicle.status,
      locationId: vehicle.locationId,
      lastSeenAt: vehicle.lastSeenAt ? vehicle.lastSeenAt.toISOString() : null,
      source: vehicle.source ?? null,
      freshnessState: vehicle.freshnessState ?? null,
    });
  }
  ...
  // ─────────────────────────────
  async holdVehicle(tenantId: string, vin: string) {
