...
export class OnboardingService {
  ...
  async onboard(dto: OnboardDealershipDto, actorUserId?: string) {
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      const locPublic = {
        ...(typeof location?.publicConfig === 'object' && location?.publicConfig
      ...
      if (!location) {
        location = await tx.location.create({
          data: {
            ...
            storeHours: (dto.storeHours as Prisma.InputJsonValue) ?? undefined,
        ...
        location = await tx.location.update({
          ...
          data: {
            ...
            publicConfig: locPublic as Prisma.InputJsonValue,
            inventoryFeedUrl: dto.inventoryFeedUrl ?? location.inventoryFeedUrl,
